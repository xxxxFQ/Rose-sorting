...
#include "logging.h"
#include "utils.h"
//#include "D:/VSProject/tensorrtx-yolov5-v7.0/yolov5/src/model.h"
//#include "D:/VSProject/tensorrtx-yolov5-v7.0/yolov5/src/config.h"
#include <fstream>
#include <iostream>
#include <chrono>
#include <cmath>
...
std::string colorSort() {
    cudaSetDevice(kGpuId);
    std::string wts_name = "";
    std::string engine_name = "D:/VSProject/tensorrtx-yolov5-v7.02/tensorrtx-yolov5-v7.0/yolov5/build/Release/color_best.engine";
    float gd = 0.0f, gw = 0.0f;
    std::string img_dir = "D:/roseSorting/MultipleCamera/color/";
    ...
    auto classes = read_classes("D:/roseSorting/MultipleCamera/weights/imagenet_classes.txt");

    // batch predict
    for (int i = 0; i < file_names.size(); i += kBatchSize) {
        // Get a batch of images
        std::vector<cv::Mat> img_batch;
        std::vector<std::string> img_name_batch;
        for (int j = i; j < i + kBatchSize && j < file_names.size(); j++) {
        ...
        for (int b = 0; b < img_name_batch.size(); b++) {
            ...
            for (int idx = 0; idx < 3;idx++) {
                std::cout << res[idx]<<"   ";
                std::cout << classes[idx] << std::endl;
                if (res[idx] >= colorCon) {
                    colorCon = res[idx];
                    maxConIndex = idx;
                }
            }
            std::cout << "Max scort"<<colorCon << "label index:"<<maxConIndex<< std::endl;
            std::cout << "return: "<<classes[maxConIndex] << std::endl;
            //return classes[maxConIndex];
        }
    }
