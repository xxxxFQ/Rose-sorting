...
if __name__ == '__main__':
    root = 'data/color_2'
    class_color_dir = os.listdir(root)
    save_root = 'data/color'
    if os.path.exists(save_root) is True:
        shutil.rmtree(save_root)
        os.mkdir(save_root)
    for class_color in class_color_dir:
        ''' 划分数据集 '''
        # file_root = f'{root}/new_txt'
        image_root = f'{root}/{class_color}'
        img_train_save_root = f'{save_root}/train/{class_color}'
        # txt_train_save_root = img_train_save_root.replace('images', 'labels')
        img_val_save_root = f'{save_root}/val/{class_color}'
        # txt_val_save_root = img_val_save_root.replace('images', 'labels')
        # img_test_save_root = f'{save_root}/test/{class_color}'
        # txt_test_save_root = img_test_save_root.replace('images', 'labels')
        if os.path.exists(img_train_save_root) is False:
            os.makedirs(img_train_save_root)
            os.makedirs(img_val_save_root)
            # os.makedirs(img_test_save_root)
            # os.makedirs(txt_train_save_root)
            # os.makedirs(txt_val_save_root)
            # os.makedirs(txt_test_save_root)

        # filelist = os.listdir(file_root)
        filelist = os.listdir(image_root)
        # train, val, test = split_array(filelist)
        train, val = split_array(filelist)

        # print("Train array:", train)
        # print("Validation array:", val)
        # print("Test array:", test)
        for i in filelist:
            if i.endswith('.jpg'):
                if i in train:
                    # shutil.move(f'{file_root}/{i}', f'{txt_train_save_root}/{i}')
                    shutil.copy(f'{image_root}/{i}', f'{img_train_save_root}/{i}')
                    # img_file = f'{image_root}/{i}'.replace('txt', 'jpg')
                    # img = cv2.imread(img_file)
                    # img_resize = cv2.resize(img, (0,0), fx=0.677, fy=0.677)
                    # cv2.imwrite(f'{img_train_save_root}/{i}'.replace('txt', 'jpg'), img_resize)
                elif i in val:
                    # shutil.move(f'{file_root}/{i}', f'{txt_val_save_root}/{i}')
                    shutil.copy(f'{image_root}/{i}', f'{img_val_save_root}/{i}')
                    # img_file = f'{image_root}/{i}'.replace('txt', 'jpg')
                    # img = cv2.imread(img_file)
                    # img_resize = cv2.resize(img, (0,0), fx=0.677, fy=0.677)
                    # cv2.imwrite(f'{img_val_save_root}/{i}'.replace('txt', 'jpg'), img_resize)
